...
int AmlogicPlayerStreamSource::feedMoreData(int datasize)
{
	...
	if((datasize>0) ||(oldneedsize>0 && datasize<=0)){/*have underflow,may need wake it,*/
		///LOGI("AmlogicPlayerStreamSource::feedMoreData new=%d,old=%d,pos=%lld\n",datasize,oldneedsize,pos);
...
int AmlogicPlayerStreamSource::Source_read(unsigned char *buf, int size)
{
    ...
    int readlen=0;
#define READ_DROPDATA (188*10*8)

    while(mNeedMoreDataSize<=0 && dropdatalen<=0){
    ...
    oncereadmax=(oncereadmax>mNeedMoreDataSize)?mNeedMoreDataSize:oncereadmax;
    if(dropdatalen>0){
